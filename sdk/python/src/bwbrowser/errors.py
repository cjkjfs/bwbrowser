"""Exceptions raised by the BW Browser SDK.
...
class BwbrowserError(Exception):
    """Base class for everything this package raises."""


class BwbrowserConnectionError(BwbrowserError):
...
class BwbrowserAPIError(BwbrowserError):
...
class ServerError(BwbrowserAPIError):
...
_BY_STATUS: dict[int, type[BwbrowserAPIError]] = {
    cls.status: cls
    for cls in (
        ValidationError,
        Unauthorized,
...
def error_for_status(
    ...
    *,
    ...
    A status with no class of its own becomes a plain :class:`BwbrowserAPIError`,
    ...
    if cls is None:
        cls = ServerError if status >= 500 else BwbrowserAPIError
