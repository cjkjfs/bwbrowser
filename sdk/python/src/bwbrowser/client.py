...
from .errors import BwbrowserConnectionError, BwbrowserError, error_for_status
from .models import (
    AgentClick,
    AgentTyping,
    ApiGroupResponse,
...
__all__ = ["BwbrowserClient", "RunSession", "DEFAULT_PORT", "DEFAULT_HOST"]
...
class BwbrowserClient:
    ...
    * ``token`` falls back to ``BWBROWSER_API_TOKEN``.
    * ``port`` falls back to ``BWBROWSER_API_PORT``, then to ``10108``.
    * ``base_url``, when given, overrides ``host`` and ``port`` entirely.
    ...
    def __init__(
        ...
        *,
        ...
        resolved_token = token if token is not None else environment.get("BWBROWSER_API_TOKEN")
        if not resolved_token:
            raise BwbrowserError(
                "No API token. Pass token=..., or set BWBROWSER_API_TOKEN. The token is "
        ...
        if base_url:
            ...
            if parts.scheme not in ("http", "https"):
                raise BwbrowserError(f"base_url must be http or https, got {parts.scheme!r}")
            ...
            if resolved_port is None:
                raw_port = environment.get("BWBROWSER_API_PORT")
                if raw_port:
                    try:
                        resolved_port = int(raw_port)
                    except ValueError as invalid:
                        raise BwbrowserError(
                            f"BWBROWSER_API_PORT is not a number: {raw_port!r}"
                        ) from invalid
            self.scheme = "http"
    ...
    def __enter__(self) -> "BwbrowserClient":
        return self
    ...
    def _request(
        ...
        *,
        ...
        with self._lock:
            ...
            # and reporting that as "Bwbrowser is unreachable" would be a lie.
            for attempt in (0, 1):
                ...
                try:
                    ...
                    if reused and attempt == 0:
                        continue
                    raise BwbrowserConnectionError(
                        f"Could not reach BW Browser at {self.base_url} "
    ...
    def agent_pick(self, profile_id: str, *, timeout_ms: Optional[int] = None) -> PickedElement:
        ...
        :class:`~bwbrowser.errors.RequestTimeout`.
        """
        return self._request(
            "POST",
    ...
    def import_proxies(
        ...
        *,
        ...
        ``format`` is ``"txt"`` (one proxy per line) or ``"json"`` (a Bwbrowser
        proxy export).
        """
        return self._request(
            "POST",
...
class RunSession:
    ...
    def __init__(
        ...
        *,
        ...
        self.cleanup_error: Optional[BwbrowserError] = None
    ...
    def cdp_url(self) -> str:
        ...
        if self.remote_debugging_port is None:
            raise BwbrowserError("The session is not running: enter the `with` block first.")
    ...
    def __exit__(
        ...
        except BwbrowserError as failure:
