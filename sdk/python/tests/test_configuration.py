...
from bwbrowser import DEFAULT_HOST, DEFAULT_PORT, BwbrowserClient, BwbrowserError


def test_arguments_are_used_as_given() -> None:
    client = BwbrowserClient(token="from-argument", port=12345, env={})
...
def test_the_environment_fills_in_what_was_not_passed() -> None:
    client = BwbrowserClient(env={"BWBROWSER_API_TOKEN": "from-env", "BWBROWSER_API_PORT": "13579"})
...
def test_the_port_falls_back_to_the_app_default() -> None:
    client = BwbrowserClient(env={"BWBROWSER_API_TOKEN": "t"})
    assert client.port == DEFAULT_PORT == 10108


def test_a_base_url_overrides_host_and_port() -> None:
    client = BwbrowserClient(
        base_url="http://127.0.0.1:9999/bwbrowser",
        token="t",
        env={"BWBROWSER_API_PORT": "222"},
    )
    assert client.port == 9999
    assert client.base_url == "http://127.0.0.1:9999/bwbrowser"


def test_a_base_url_prefix_is_kept_on_every_path(fake: FakeBwbrowser) -> None:
    with BwbrowserClient(
        base_url=f"http://127.0.0.1:{fake.port}/bwbrowser", token="t", timeout=5.0, env={}
    ) as client:
        client.list_profiles()
    assert fake.last.path == "/bwbrowser/v1/profiles"


def test_an_unusable_port_in_the_environment_is_reported() -> None:
    with pytest.raises(BwbrowserError) as raised:
        BwbrowserClient(env={"BWBROWSER_API_TOKEN": "t", "BWBROWSER_API_PORT": "not-a-number"})
    assert "BWBROWSER_API_PORT" in str(raised.value)


def test_an_unsupported_scheme_is_refused() -> None:
    with pytest.raises(BwbrowserError):
        BwbrowserClient(base_url="ftp://127.0.0.1:9999", token="t", env={})


def test_the_websocket_address_is_built_from_the_same_base() -> None:
    client = BwbrowserClient(token="t", port=10108, env={})
    assert (
...
def test_a_reopened_client_still_works(fake: FakeBwbrowser) -> None:
    """`close()` drops the socket; the next call has to open a new one."""
    with BwbrowserClient(token="t", port=fake.port, timeout=5.0, env={}) as client:
        client.list_profiles()
        client.close()
        client.list_profiles()
