/**
 * Shared physics for the Bwbrowser logo easter eggs: clones an element into a
...
export interface BwbrowserLaunchOptions {
 ...
 */
export function launchBwbrowserClone(
  el: HTMLElement,
  options: BwbrowserLaunchOptions = {},
): () => void {
  ...
  // The fallen bwbrowser is a toy: it can be grabbed 1:1 and thrown, inheriting
  ...
  const onPointerDown = (e: PointerEvent) => {
    ...
    // Respect where the bwbrowser was grabbed — no snap to center.
  ...
  const onPointerUp = (e: PointerEvent) => {
    ...
    // throws the bwbrowser instead of dropping it.
    const now = performance.now();
  ...
  const animate = (time: number) => {
    ...
    if (!grabbed) {
      ...
      if (currentBottom >= floorY && vy > 0) {
        ...
        // A bwbrowser dropped with no sideways speed would hop in place forever —
      ...
      // damping), and keep rolling. Left wall has no bounce — the bwbrowser
