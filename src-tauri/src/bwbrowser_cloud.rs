...
fn bwbrowser_log_path() -> Option<std::path::PathBuf> {
  let app_name = crate::app_dirs::app_name();
  let base = std::env::var_os("LOCALAPPDATA")
    .map(std::path::PathBuf::from)
    .unwrap_or_else(|| std::path::PathBuf::from("./"));
  let dir = base.join(app_name);
  Some(dir.join("bwbrowser_debug.log"))
}
    ...
    loop {
      ...
      if !is_running {
        ...
        let cookie_str = match crate::cookie_manager::CookieManager::export_cookies(
          ...
          Ok(_) => {
            log_bwbrowser("vps_cookie_sync", "SQLite cookie 为空，尝试 CDP");
            match crate::cookie_sync::export_cookies_via_cdp(&profile).await {
              Ok(s) => s,
              Err(e) => {
                log_bwbrowser_error(
                  "vps_cookie_sync",
                  &format!("导出 cookie 失败（SQLite 和 CDP 均失败）: {}", e),
                );
                String::new()
              }
            }
          ...
          Err(e) => {
            ...
            match crate::cookie_sync::export_cookies_via_cdp(&profile).await {
              ...
              Err(e2) => {
                log_bwbrowser_error(
                  "vps_cookie_sync",
                  &format!("导出 cookie 失败（SQLite: {}, CDP: {}）", e, e2),
                );
                String::new()
              }
            }
...
pub async fn bwbrowser_update_account_proxy(
...

  {
    ...
    if matching_profiles_was_empty {
      log_bwbrowser_error(
        "update_account_proxy",
        &format!(
          "  ⚠ 未找到匹配的本地 profile: name={}（启动时会从云端拉取代理配置）",
          profile_name
        ),
      );
    }
  ...
  if !host.is_empty() {
    ...
    // No local proxy lookup needed.
    let geo_info: ProxyGeoInfo = async {
      // 方案1：直接用 proxy_node 解析的设置检测代理出口 IP 时区
        ...
        if let Some(cp) = matching {
            ...
            .await
          {
            Ok(_) => {
              log_bwbrowser(
                "update_account_proxy",
                &format!("  ✓ 时区已回传到服务器: timezone={}", geo.timezone),
              );
            }
            Err(e) => {
              log_bwbrowser(
...
pub async fn bwbrowser_launch_account(
  ...
  //     云端 API 可能因服务器同步延迟返回旧值，导致覆盖本地正确配置。
  ...
  //     - 全新 profile（无 Cookie 文件）：可以安全注入云端 Cookie
  ...
  //    云端账号的指纹在启动时通过代理出口 IP 自动解析时区/语言
