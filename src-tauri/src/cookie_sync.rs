...
pub async fn inject_cookies_via_cdp(
  ...
  // Only navigate when a real target URL is provided; avoid navigating to a blank
  // page when none is given (the cookie is written and effective on the current tab).
  if let Some(nav_target) = nav_url {
    let _ = conn
