...
/// Setup function to ensure bwbrowser-proxy binary exists and cleanup stale proxies
async fn setup_test() -> Result<std::path::PathBuf, Box<dyn std::error::Error + Send + Sync>> {
  ...
  // Build bwbrowser-proxy binary if it doesn't exist
      ...
      .is_ok_and(|output| {
        output.status.success()
          && String::from_utf8_lossy(&output.stdout).trim()
            == format!("bwbrowser-proxy {}", env!("BUILD_VERSION"))
      });

  if !binary_is_current {
    println!("Building bwbrowser-proxy binary for integration tests...");
...
async fn test_sidecar_reports_build_version() -> Result<(), Box<dyn std::error::Error + Send + Sync>>
{
  ...
  assert!(
    output.status.success(),
    "bwbrowser-proxy --version failed: {}",
...
impl XrayChainCleanup {
  ...
  async fn cleanup(&mut self) {
    if let Some(id) = self.outer_id.take() {
      let _ = bwbrowser_lib::proxy_runner::stop_proxy_process(&id).await;
    }
    if let Some(id) = self.xray_id.take() {
      let _ = bwbrowser_lib::xray_worker_runner::stop_xray_worker(&id).await;
    }
  }
}
...
impl Drop for XrayChainCleanup {
  fn drop(&mut self) {
    ...
    if let Some(id) = self.outer_id.take() {
      if let Some(config) = bwbrowser_lib::proxy_storage::get_proxy_config(&id) {
        if let Some(pid) = config.pid {
          terminate_process_tree(pid);
        }
      }
      bwbrowser_lib::proxy_storage::delete_proxy_config(&id);
    }
    if let Some(id) = self.xray_id.take() {
      if let Some(config) = bwbrowser_lib::xray_worker_storage::get_xray_worker_config(&id) {
        if let Some(pid) = config.pid {
          terminate_process_tree(pid);
        }
      }
      bwbrowser_lib::xray_worker_storage::delete_xray_worker_config(&id);
    }
  }
}
...
/// server. Set `BWBROWSER_E2E_VLESS_URI`; optionally set
/// `BWBROWSER_E2E_XRAY_TARGET_URL` to a controlled plain-HTTP endpoint. When the
...
async fn test_xray_reality_chain_and_local_traffic_monitoring(
  ...
  let Some(vless_uri) = std::env::var("BWBROWSER_E2E_VLESS_URI")
    .ok()
    .filter(|value| !value.is_empty())
  else {
    println!("skipping Xray-core integration test: BWBROWSER_E2E_VLESS_URI is not set");
    return Ok(());
  };

  let parsed = bwbrowser_lib::xray::parse_vless_uri(&vless_uri)?;
  ...
  let target_url = if let Ok(url) = std::env::var("BWBROWSER_E2E_XRAY_TARGET_URL") {
    url
  ...
  if target.scheme() != "http" {
    return Err(
      "BWBROWSER_E2E_XRAY_TARGET_URL must use plain HTTP for exact payload accounting".into(),
    );
  }
  ...
  let _data_root = EnvironmentRestore::set_path("BWBROWSER_DATA_ROOT", isolated_root.path());
  let _binary_path = setup_test().await.map_err(|error| error.to_string())?;
  let profile_id = format!("xray-integration-{}", uuid::Uuid::new_v4());
  let payload = b"bwbrowser-xray-monitor-payload";
  let mut cleanup = XrayChainCleanup::new();

  let (first_worker, concurrent_worker) = tokio::join!(
    bwbrowser_lib::xray_worker_runner::start_xray_worker(Some(&profile_id), &vless_uri),
    bwbrowser_lib::xray_worker_runner::start_xray_worker(Some(&profile_id), &vless_uri),
  );
  ...
  let mut outer = bwbrowser_lib::proxy_runner::start_proxy_process_with_profile(
    Some(upstream_url),
    None,
  ...
  assert!(
    bwbrowser_lib::proxy_storage::update_proxy_config(&outer),
  ...
  if endpoint_is_loopback && std::env::var_os("BWBROWSER_E2E_XRAY_TARGET_URL").is_none() {
  ...
  let stats = tokio::time::timeout(Duration::from_secs(10), async {
    loop {
      if let Some(stats) = bwbrowser_lib::traffic_stats::load_traffic_stats(&profile_id) {
  ...
  assert!(bwbrowser_lib::proxy_storage::get_proxy_config(&outer_id).is_none());
  assert!(bwbrowser_lib::xray_worker_storage::get_xray_worker_config(&xray_id).is_none());
  assert!(!bwbrowser_lib::xray_worker_storage::xray_runtime_config_path(&xray_id).exists());
  for pid in [outer_pid, supervisor_pid, xray_pid].into_iter().flatten() {
    for _ in 0..20 {
      if !bwbrowser_lib::proxy_storage::is_process_running(pid) {
        break;
      }
      sleep(Duration::from_millis(100)).await;
    }
    assert!(
      !bwbrowser_lib::proxy_storage::is_process_running(pid),
...
async fn test_traffic_tracking() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
  ...
  let traffic_stats_dir = bwbrowser_lib::app_dirs::cache_dir().join("traffic_stats");
...
async fn test_bypass_rules_http_direct() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
  ...
  // Start bwbrowser-proxy with upstream + bypass rules for "127.0.0.1"
  ...
  println!("Bwbrowser-proxy started on port {local_port} with bypass rules for 127.0.0.1");
...
async fn test_bypass_rules_in_config() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
  ...
  let proxies_dir = bwbrowser_lib::app_dirs::proxy_workers_dir();
...
/// Test that a SOCKS5 upstream proxy works end-to-end through bwbrowser-proxy.
/// Starts a mock SOCKS5 server, a mock HTTP target server,
/// then routes requests through bwbrowser-proxy -> SOCKS5 -> target.
#[tokio::test]
#[serial]
async fn test_local_proxy_with_socks5_upstream(
  ...
  // Test 1: HTTP request through bwbrowser-proxy -> SOCKS5 -> target
...
/// Verifies the full chain: client → bwbrowser-proxy → Shadowsocks → internet.
#[tokio::test]
#[serial]
async fn test_local_proxy_with_shadowsocks_upstream(
  ...
  let ss_container = "bwbrowser-ss-test";
  let ss_port = 18388u16;
  let ss_password = "bwbrowser-test-password";
  ...
  // Start bwbrowser-proxy with Shadowsocks upstream
  let output = tokio::process::Command::new(&binary_path)
    ...
    .env("BWBROWSER_PROXY_USERNAME", ss_method)
    .env("BWBROWSER_PROXY_PASSWORD", ss_password)
    .output()
    .await?;
  ...
  // Test: HTTP request through bwbrowser-proxy → Shadowsocks → example.com
  let mut stream = TcpStream::connect(("127.0.0.1", local_port)).await?;
  let request =
...
// ---------------------------------------------------------------------------
// Worker lifecycle: a detached bwbrowser-proxy must die with the browser it serves,
...
// BWBROWSER_PROXY_WATCHDOG_INTERVAL_MS so a reap is observable in seconds.
// ---------------------------------------------------------------------------
...
async fn wait_for_worker_exit(proxy_id: &str, timeout: Duration) -> bool {
  let deadline = std::time::Instant::now() + timeout;
  while std::time::Instant::now() < deadline {
    if bwbrowser_lib::proxy_storage::get_proxy_config(proxy_id).is_none() {
      return true;
    }
    sleep(Duration::from_millis(100)).await;
  }
...
async fn start_lifecycle_worker(
  profile_id: &str,
) -> Result<bwbrowser_lib::proxy_storage::ProxyConfig, Box<dyn std::error::Error + Send + Sync>> {
  std::env::set_var(
    "BWBROWSER_PROXY_WATCHDOG_INTERVAL_MS",
    TEST_WATCHDOG_INTERVAL_MS,
  );
  bwbrowser_lib::proxy_runner::start_proxy_process_with_profile(
    None,
    None,
...
fn claim_worker_for(proxy_id: &str, browser_pid: u32) -> bool {
  let Some(mut config) = bwbrowser_lib::proxy_storage::get_proxy_config(proxy_id) else {
    return false;
  };
  let Some(start_time) = bwbrowser_lib::proxy_storage::resolve_process_start_time(browser_pid)
  else {
    return false;
  };
  config.browser_pid = Some(browser_pid);
  config.browser_pid_start_time = Some(start_time);
  bwbrowser_lib::proxy_storage::update_proxy_config(&config)
}
...
/// bwbrowser-proxy after closing the browser".
#[tokio::test]
#[serial]
async fn worker_reaps_itself_when_its_browser_exits(
  ...
  let owner = bwbrowser_lib::proxy_storage::get_proxy_config(&config.id)
  ...
  assert!(
    bwbrowser_lib::proxy_storage::get_proxy_config(&config.id).is_some(),
  ...
  for _ in 0..100 {
    if !bwbrowser_lib::proxy_storage::is_process_running(worker_pid) {
      process_gone = true;
      break;
    }
    sleep(Duration::from_millis(100)).await;
...
async fn worker_reaps_itself_when_its_browser_pid_is_recycled(
  ...
  let mut owner = bwbrowser_lib::proxy_storage::get_proxy_config(&config.id)
    .ok_or("worker config missing after start")?;
  owner.browser_pid = Some(stub.pid());
  owner.browser_pid_start_time = Some(1);
  assert!(bwbrowser_lib::proxy_storage::update_proxy_config(&owner));
...
async fn worker_exits_when_its_config_is_deleted(
  ...
  assert!(bwbrowser_lib::proxy_storage::delete_proxy_config(
    &config.id
  ));
