 ...
 * to bwbrowser-sync; client `list()` calls strip it from results so it never
 * leaks into application data.
 */
const MANIFEST_KEY = ".bwbrowser-sync-manifest";
...
export class SyncService implements OnModuleInit {
    ...
    if (isSingleLabel) {
      this.logger.warn(
        `Storage endpoint '${this.publicEndpoint}' uses the container-only host '${host}'. ` +
          "Presigned URLs built from it cannot be reached by BW Browser, so every " +
  ...
  async list(dto: ListRequestDto, ctx?: UserContext): Promise<ListResponseDto> {
    ...
    const objects = (response.Contents || [])
      // Don't leak bwbrowser-sync's internal manifest object to clients.
